...
const Goals = () => {
  ...
  const handleSubmit = (e) => {
    ...
    const newGoal = addGoal({
      ...
      achieved: formData.metrics.every(m => m.current >= m.target),
    });
                        ...
                        return (
                          <div key={idx} className="mb-4">
                            <div className="flex justify-between text-sm font-semibold mb-2">
                              <span className="text-red-300">{m.name}: {m.current} / {m.target} {m.unit}</span>
          ...
          {achievedGoals.length > 0 && (
            <div>
              ...
              <div className="space-y-3">
                {achievedGoals.reverse().slice(0, 5).map(goal => {
                  const typeInfo = allGoalTypes[goal.type] || allGoalTypes.custom;
                  return (
                    <motion.div
                      key={goal.id}
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      setFormData({
                        ...formData,
                        type,
                        metrics: [{ name: type.charAt(0).toUpperCase() + type.slice(1), current: 0, target: 0, unit: type === 'weight' || type === 'strength' ? 'kg' : type === 'endurance' ? 'min' : '' }],
                      });
                    }}
                ...
                {formData.metrics.map((m, idx) => (
                  <div key={idx} className="grid grid-cols-3 gap-2">
                    <input
                      ...
                      className="col-span-1 px-3 py-2 bg-dark-700 border border-red-500/30 text-white rounded-xl outline-none"
                    />
                    <input
                      type="number"
                      placeholder="Current"
                      ...
                      className="px-3 py-2 bg-dark-700 border border-red-500/30 text-white rounded-xl outline-none"
                    />
                    <input
                      type="number"
                      placeholder="Target"
                ...
                <button
                  ...
                  onClick={() => setFormData({
                    ...formData,
                    metrics: [...formData.metrics, { name: '', current: 0, target: 0, unit: '' }]
                  })}
